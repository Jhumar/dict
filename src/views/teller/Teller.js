...
function Teller() {
  ...
  const next = () => {
    ...
    if (isRunning) {
      return alert('The queue is still running, please end the current queue to proceed.');
    }
    ...
    if (currentQueue) {
      return alert('There is still queue pending, complete it first before requesting.');
    }
    ...
    <>
      ...
      <Container className='py-5'>
        <Row>
            ...
            <Row>
              <Col className='mb-4' lg={8}>
                <Card ref={cardQueueDOM} className='p-4'>
                  <Card.Body className="text-center">
                    <h2 className='fs-5 fw-bold text-warning'>CURRENT SERVING</h2>
                    ...
                    <Card.Text>
                      <span className='h1 text-warning border border-3 border-warning rounded my-5 p-5 d-inline-block'>
                        {(currentQueue && currentQueue.number ? currentQueue.number : '--')}
                      </span>
              ...
              <Col className='mb-4' lg={4}>
                <div className="d-flex flex-wrap w-100 h-100">
                  <Button className='d-block w-100 mb-3 py-3' variant='primary' onClick={next}>Next</Button>
                  <Button className='d-block w-100 mb-3 py-3' variant='primary'>Recall</Button>
                  <Button className='d-block w-100 mb-3 py-3' variant='primary' onClick={start}>Start</Button>
                  <Button className='d-block w-100 py-3' variant='primary' onClick={end}>End</Button>
                </div>
              </Col>
            </Row>
          </Col>
          <Col lg={4}>
            <div>
              <ListGroup className='scrollable-list-group' style={{maxHeight: queueListHeight}}>
                <ListGroup.Item className='fs-5 fw-bold text-center py-4 sticky-top' variant='dark'>{ user && user.window && user.window.name }</ListGroup.Item>
                {(queues || []).length > 0 ? (queues || []).map(q => {
                  return (
                    <ListGroup.Item key={q.uuid}>Queue #{q.number}</ListGroup.Item>
                  )
                }): (
